...
async def run_regenerate_script_crew(inputs: dict) -> str:
    try:
        ...
        if output_path.exists():
            return output_path.read_text()
        raise FileNotFoundError("Refined script output file not generated")
    except Exception as e:
        raise RuntimeError(f"Script refinement failed: {str(e)}")
    finally:
        os.chdir(Path(__file__).parent)
...
async def refine_script(request: RefineRequest):
    try:
        inputs = request.dict()
        refined_script_output = await run_regenerate_script_crew(inputs)
        return {"success": True, "script": refined_script_output}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e)) 